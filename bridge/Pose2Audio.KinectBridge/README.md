# Pose2Audio.KinectBridge

UDP pose bridge for `Pose2Audio` `kinectv2` provider.
...
```powershell
dotnet run --project bridge\Pose2Audio.KinectBridge -- --mode mock --host 127.0.0.1 --port 24567 --hz 60 --duration-sec 0
```

Use non-scrolling live console output (default):

```powershell
dotnet run --project bridge\Pose2Audio.KinectBridge -- --mode mock --cli-output live
```

Print HRTF az/el against a chosen world source:

```powershell
dotnet run --project bridge\Pose2Audio.KinectBridge -- --mode mock --source-x 0 --source-y 0 --source-z 1
```

Kinect SDK mode now uses dedicated project:

```powershell
dotnet run --project bridge\Pose2Audio.KinectBridge.KinectSdk -- --host 127.0.0.1 --port 24567 --hz 60 --duration-sec 0
```
...
- `kinect-sdk`: delegated to `Pose2Audio.KinectBridge.KinectSdk` project.
...
```powershell
echo {"tracked":true,"position_m":[0,0,0],"quaternion_wxyz":[1,0,0,0]} | dotnet run --project bridge\Pose2Audio.KinectBridge -- --mode stdin-json
```
...
1. Real Kinect SDK capture lives in `bridge/Pose2Audio.KinectBridge.KinectSdk`.
