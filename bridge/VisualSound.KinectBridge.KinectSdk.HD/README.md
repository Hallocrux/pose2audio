# VisualSound.KinectBridge.KinectSdk.HD
...
```powershell
dotnet run --project bridge\VisualSound.KinectBridge.KinectSdk.HD -- --host 127.0.0.1 --port 24567 --hz 60 --duration-sec 0
```

Use non-scrolling live console output (default):

```powershell
dotnet run --project bridge\VisualSound.KinectBridge.KinectSdk.HD -- --cli-output live
```

Print HRTF az/el against a chosen world source:

```powershell
dotnet run --project bridge\VisualSound.KinectBridge.KinectSdk.HD -- --source-x 0 --source-y 0 --source-z 1
```
