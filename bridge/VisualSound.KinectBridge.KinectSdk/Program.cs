...
namespace VisualSound.KinectBridge.KinectSdk
{
    ...
    internal sealed class KinectSdkBridge : IDisposable
    {
        ...
        private readonly FaceFrameSource _faceSource;
        private readonly FaceFrameReader _faceReader;
        ...
        public KinectSdkBridge(BridgeOptions options)
        {
            ...
            _faceSource = new FaceFrameSource(_sensor, 0, FaceFrameFeatures.RotationOrientation);
            _faceReader = _faceSource.OpenReader();
            if (_faceReader == null)
            {
                throw new InvalidOperationException("Failed to open FaceFrameReader.");
            }
        ...
        {
            Console.WriteLine(
                "[{0}] kinect-sdk bridge start udp={1}:{2} hz={3} duration={4}",
        ...
        private void PollKinect()
        {
            using (var bodyFrame = _bodyReader.AcquireLatestFrame())
            {
                if (bodyFrame != null)
                {
                    ...
                    else
                    {
                        ...
                        if (!_faceSource.IsTrackingIdValid || _faceSource.TrackingId != _trackedBodyId)
                        {
                            _faceSource.TrackingId = _trackedBodyId;
                            _hasFaceQ = false;
                        }
            ...
            if (_trackedBodyId == 0 || !_faceSource.IsTrackingIdValid)
            {
                _hasFaceQ = false;
                return;
            }

            using (var faceFrame = _faceReader.AcquireLatestFrame())
            {
                ...
                var fq = faceFrame.FaceFrameResult.FaceRotationQuaternion;
        ...
        private void ComposeLatestSample()
        {
            ...
            var q = _hasFaceQ ? _faceQ : (_hasBodyQ ? _bodyQ : IdentityQuaternion());
...
@"Usage:
  VisualSound.KinectBridge.KinectSdk [--host 127.0.0.1] [--port 24567] [--hz 60] [--duration-sec 0] [--source-x 0] [--source-y 0] [--source-z 1] [--cli-output live|scroll]
